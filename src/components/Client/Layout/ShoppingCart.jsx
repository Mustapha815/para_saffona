...
const ShoppingCart = () => {
  ...
  const createOrderMutation = useMutation({
    ...
    onError: (error) => {
      ...
      setIsProcessing(false); // Reset loading state on error too
    }
  });
                ...
                {currentStep === 'review' && (
                  <>
                    ...
                    <button 
                      ...
                      disabled={isProcessing}
                      className="w-full border border-gray-300 text-gray-700 py-3 px-4 rounded-lg hover:bg-gray-50 disabled:bg-gray-100 disabled:text-gray-400 transition-colors text-sm sm:text-base"
                    >
                      {t('backToInformation')}
                    </button>
                  </>
