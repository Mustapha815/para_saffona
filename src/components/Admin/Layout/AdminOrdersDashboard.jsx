...
import { 
  Search, Filter, Eye, Edit, CheckCircle, XCircle, Clock, User, Phone, MapPin, Package, DollarSign,
  Calendar, TrendingUp, X, ChevronLeft, ChevronRight, ShoppingBag,
  Truck, CreditCard, FileText, Mail, Home, ArrowLeft, ArrowRight
} from 'lucide-react';
...
const AdminOrdersDashboard = () => {
  ...
  const getStatusIcon = (status) => {
    const icons = {
      ...
      returned: <Edit className="w-4 h-4" />,
  ...
  // Stats cards data
  const statsCards = [
    { title: 'Total Orders', value: stats.total, icon: Package, gradient: 'from-blue-500 to-indigo-500', change: '+12.5%', changeType: 'positive' },
    { title: 'Pending Orders', value: stats.pending, icon: Clock, gradient: 'from-orange-500 to-red-500', change: '+3.2%', changeType: 'positive' },
    { title: 'Delivered Orders', value: stats.delivered, icon: CheckCircle, gradient: 'from-emerald-500 to-teal-500', change: '+8.7%', changeType: 'positive' },
    { title: 'Total Revenue', value: formatCurrency(stats.revenue), icon: DollarSign, gradient: 'from-purple-500 to-pink-500', change: '+15.3%', changeType: 'positive' }
  ];
          ...
          <div key={idx} className="group relative overflow-hidden">
            ...
            <div className="relative bg-white/70 backdrop-blur-xl rounded-2xl p-6 shadow-lg border border-white/50 hover:shadow-xl transition-all duration-300 group-hover:-translate-y-1">
              <div className="flex items-start justify-between mb-4">
                ...
                <div className={`flex items-center space-x-1 px-2 py-1 rounded-full text-xs font-medium ${stat.changeType === 'positive' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-700'}`}>
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">{t('vs last month')}</p>
              </div>
            </div>
          </div>
        );
    ...
    <div className="bg-white/70 backdrop-blur-xl rounded-2xl shadow-lg border border-white/50 p-6">
      <div className="flex flex-col md:flex-row gap-4 items-center">
        <div className="flex-1 relative">
          ...
          <input
            type="text"
            placeholder={t('Search orders by order number, name, or phone...')}
            value={searchTerm}
    ...
    <div className="bg-white/70 backdrop-blur-xl rounded-2xl shadow-lg border border-white/50 overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('Order Number')}</th>
              <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('Customer')}</th>
              <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('Items')}</th>
              <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('Total')}</th>
              <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('Status')}</th>
              <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('Date')}</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {paginatedOrders.map(order => (
              <tr key={order.id} className="hover:bg-gray-50/50 transition-colors duration-200">
                ...
                <td className="px-6 py-4 whitespace-nowrap">{order.order_items?.length || 0} {t('Items')}</td>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium flex items-center gap-2">
                  ...
                  >
                    <option value="pending">{t('Pending Orders')}</option>
                    <option value="returned">{t('Returned Orders')}</option>
                    <option value="cancelled">{t('Cancelled Orders')}</option>
                    <option value="delivered">{t('Delivered Orders')}</option>
                  </select>
                </td>
              </tr>
            ))}
      ...
      {filteredOrders.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 mt-4">{t('No orders found')}</p>
        </div>
      )}
        ...
        <div className="px-6 py-4 border-t border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <span className="text-sm text-gray-700">
              {t('Showing')} {((currentPage - 1) * itemsPerPage) + 1} {t('to')} {Math.min(currentPage * itemsPerPage, filteredOrders.length)} {t('of')} {filteredOrders.length} {t('results')}
            </span>
            <select
              value={itemsPerPage}
              onChange={(e) => {
                        ...
                        <div className={`bg-gradient-to-br from-violet-500 to-purple-500 ${item.target_name === 'product' ? 'p-0' : 'p-2  '}rounded-lg mr-4`}>
                          {
                            item.target_name === 'product' ? (
                              <img src={`${import.meta.env.VITE_IMG_BASE_URL}/${item.target.image}`} alt={item.target.name} className="object-cover w-12 h-12 " />
                            ) : (
